...
import domain.users.{CreateUser, Login, Password, User, UserId, UserName, UserRepositoryAlgebra}
...
//  )
//
//  override def create(user: CreateUser): EitherT[IO, LoginInUse, User] =
//    EitherT.fromEither(
//      if (users.add(user)) {
//        user.asRight
